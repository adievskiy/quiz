...
class SecondQuestion : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        answersRG = findViewById(R.id.answersSecondRG)
        answerOne = findViewById(R.id.answerSecondOne)
        answerTwo = findViewById(R.id.answerSecondTwo)
        answerThree = findViewById(R.id.answerSecondThree)
    ...
    private val answerClickListener = View.OnClickListener { v: View? ->
        when (v?.id) {
            R.id.answerSecondOne -> {
                CoroutineScope(Dispatchers.Main).launch {
                    wrong() }
            }
            R.id.answerSecondTwo -> {
                CoroutineScope(Dispatchers.Main).launch {
                    wrong() }
            }
            R.id.answerSecondThree -> {
                CoroutineScope(Dispatchers.Main).launch {
                    write() }
            }
        }
